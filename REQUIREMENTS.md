...
| R-294 | Create Ride is a 3-step wizard (Client & route → Schedule → Review) shown as a side pane on desktop / full-screen on mobile; client dropdown fed by the bounded `/options` endpoint; selecting a client auto-fills pickup with their home address; one unified address field autocompletes + verifies real addresses via the open-source geocoder; blocks same pickup/dropoff and a pickup time after the appointment | `RideCreatePanel.vue` + `AddressField.vue` | auto | browser: `admin-flows.spec.ts` |
